...
Nota bene, if you are using templates in your email provider (via [overrides](/docs/overrides#provider-overrides)),
all the rendering happens there, and the we ignore the email layout.

## How to compose email layouts

Please visit the Settings tab in the dashboard, then select Email Layout.
