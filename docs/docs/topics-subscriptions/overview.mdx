...
subtitle: Send more granular notifications with topics and subscriptions
---
...
Notifications from the same topic (and `category`) need to thread in an email client
...
- The ability to notify all subscribers of a topic. This allows you to build a more stateless
  ...
  recipient list when sending notifications.
- Follow/unfollow or watch/unwatch functionality in your front end. You can quickly build something
  like the GitHub Watch button (with category-level granularity) or the Youtube Subscribe button.
